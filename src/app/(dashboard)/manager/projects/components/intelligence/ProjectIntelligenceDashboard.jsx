...
import {
  Activity, AlertTriangle, ArrowLeft, BadgeIndianRupee, Brain, BriefcaseBusiness,
  Building2, CalendarClock, CheckCircle2, CircleDollarSign, Clock3, FileText,
  FolderKanban, Gauge, MapPin, RefreshCw, Search, ShieldAlert, Timer, TrendingDown,
...
export default function ProjectIntelligenceDashboard({ intelligence, refreshing, error, onRefresh }) {
  ...
  const searchResults = useMemo(() => {
    ...
    return [
      ...(project.employees || []).map((item) => ({ type: "Employee", title: item.fullName, detail: item.designation })),
      ...a.work.logs.map((item) => ({ type: "Work", title: item.title || item.description || item.employeeName || "Work log", detail: item.status })),
      ...intelligence.expenses.map((item) => ({ type: "Expense", title: item.description || item.category || "Expense", detail: `${item.employeeName || ""} ${money(item.amount)}` })),
  ...
  const predictions = [
    ["Estimated completion", date(p.estimatedCompletion), CalendarClock], ["Final project cost", money(p.finalCost), CircleDollarSign],
    ["Expected profit", money(p.expectedProfit), TrendingUp], ["Expected loss", money(p.expectedLoss), TrendingDown],
    ["Budget overrun", money(p.budgetOverrun), ShieldAlert], ["Delay probability", percent(p.delayProbability), Clock3],
    ["Labour requirement", `${p.labourRequirement} employees`, Users], ["Cash requirement", money(p.cashFlowRequirement), Wallet],
    ["Client payment risk", percent(p.clientPaymentRisk), Building2], ["Vendor payment risk", percent(p.vendorPaymentRisk), BriefcaseBusiness],
